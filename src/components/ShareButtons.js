...
const ShareButtons = ({ slug, title, emoji }) => {
  const encodedTitle = encodeURIComponent(
    `${emoji || "👾"}${title} | Bit by Bit`
  );
  const pageUrl = `https://bit-by-bit.netlify.app${slug}`;
  return (
    <Wrapper>
      <ShareTitle>SHARE</ShareTitle>
      <ShareLinks>
        <ShareLink
          href={`https://twitter.com/share?url=${pageUrl}&text=${encodedTitle}&via=und3fined-v01d`}
          rel="nofllow"
        >
          <img
            src={svgTwitterWhite}
      ...
      <GitHubLink
        href={`https://github.com/und3fined-v01d/bit-by-bit/edit/master/content/blog${slug}index.md`}
        rel="nofllow"
      >
        Edit on GitHub
      </GitHubLink>
